...
function LaunchDungeons() {
	Game.GetWord = function (type) {
		if (type == 'secret')
			return choose(['hidden', 'secret', 'mysterious', 'forgotten', 'forbidden', 'lost', 'sunk', 'buried', 'concealed', 'shrouded', 'invisible', 'elder']);
		if (type == 'ruined')
			return choose(['ancient', 'old', 'ruined', 'ravaged', 'destroyed', 'collapsed', 'demolished', 'burnt', 'torn-down', 'shattered', 'dilapidated', 'abandoned', 'crumbling', 'derelict', 'decaying']);
		if (type == 'magical')
			return choose(['arcane', 'magical', 'mystical', 'sacred', 'honed', 'banished', 'unholy', 'holy', 'demonic', 'enchanted', 'necromantic', 'bewitched', 'haunted', 'occult', 'astral']);
		return '';
	};
	...
	*/
	...
	for (var i in Game.Monsters) {
		if (Game.Monsters[i].boss)
			Game.BossMonsters.push(Game.Monsters[i]);
	}
	...
	Game.Entity = function (type, subtype, dungeon, pic, stats) {
		...
		else if (this.type == 'destructible') //crates, doors
		{
			...
			if (this.subtype == 'door')
				...
				this.pic = [Math.floor(Math.random() * 4 + 2), 7];
		...
		this.Say = function (what) {
			if (this.type == 'monster') {
				if (Game.Monsters[this.subtype].quotes[what])
					this.dungeon.Log(this.subtype + ' : "<span style="color:#f96;">' + choose(Game.Monsters[this.subtype].quotes[what].split('|')) + '</span>"');
			}
		};
		this.Draw = function () {
			var name = '?';
			...
			if (this.type == 'item' && this.subtype == 'cookies' && this.value > 0) {
				if (this.value < 2)
					...
					this.pic = [6, 6]; // if (this.value<1000) this.pic=[1,5];
			}
		...
		this.GoTo = function (x, y) {
			...
			if (!this.Move()) //really stuck? try to maneuver laterally!
			{
				this.targets = [];
				if (this.x == x)
					this.targets.push([1, 0], [-1, 0]); //somehow this feels inverted... but it doesn't work the other way
				if (this.y == y)
					this.targets.push([0, 1], [0, -1]); //hypothesis : *MAGIC*
				this.Move();
			}
		};
		...
		this.HitBy = function (by) {
			if (this.type == 'destructible' && by.type == 'hero') //break destructibles
			{
				...
				if (this.life <= 0) {
					...
					this.pic = [this.pic[0], this.pic[1] + 1];
			}
			else if (this.type == 'special' && this.subtype == 'upgrade') //upgrade relic
			{
				this.obstacle = 0;
				if (Game.Upgrades[this.value])
					Game.Upgrades[this.value].earn();
				this.value = '';
			}
			else if ((this.type == 'monster' && by.type == 'hero') || (this.type == 'hero' && by.type == 'monster') && this.stats.hp > 0) //it's a fight!
			{
				...
				var defenderName = '';
				if (by.type == 'hero')
					attackerName = Game.Heroes[by.subtype].name;
				else if (by.type == 'monster')
					attackerName = Game.Monsters[by.subtype].name;
				if (this.type == 'hero')
					defenderName = Game.Heroes[this.subtype].name;
				...
				if (this.stats.hp <= 0) //die
				{
					...
					if (this.type == 'monster' && by.type == 'hero') {
						...
						if (Math.random() < 0.05)
							Game.Heroes[by.subtype].Say('win');
				...
				for (var i = 0; i < howManyTurns; i++) {
					if (1 == 1) //this.AI!='static')
					{
						if (this.AI == 'flee')
							...
							if (this.stuck || this.targets.length == [])
								this.Wander(); //can't reach the player? walk around randomly
						}
					}
				}
			}
	...
	Game.Dungeon = function (type, id) {
				...
				if (tile != -1) {
					...
					if (Math.random() < 0.2) //2 in 10 spawns are monsters
					{
						...
						for (var ii in Game.Monsters) {
							...
							if (me.level != 0 && me.level <= (depth + this.level) && Math.random() < (me.stats.rarity || 1))
					...
					else //the rest of the spawns are destructibles or loot
					{
						if (Math.random() < 0.6) {
							...
							this.AddEntity('destructible', 'random', tile[0], tile[1]); //random crates etc
						this.map.removeFreeTile(tile[0], tile[1]);
					}
				}
			}
		...
		this.Turn = function () {
			for (var i in this.entities) {
				if (this.entities[i] && this.entities[i].type)
					this.entities[i].Turn();
			}
	...
	Game.DungeonLinkLocationChains = function (start, end) {
		...
		*/
		...
		if (start[0].id == end[0].id)
			return start[start.length - 1]; //same room
		for (var i in end) {
			if (start[0] == end[i].parent)
				return end[i]; //inferior branch, go to the inferior room
		}
		if (start.length > 1)
			return start[1]; //different or superior branch, go to the superior room
		...
		this.EachFrame = function () {
			if (this.dungeon.auto) {
				...
				if (this.dungeon.timer == 0) {
					...
					if (this.dungeon.timerWarmup > 0)
						this.dungeon.timerWarmup--;
	...
	Game.Hero = function (name, pic, portrait, icon) {
		...
		this.Say = function (what) {
			if (this.dialogue[what])
				Game.Dungeons[this.inDungeon].Log(this.name + ' : "<span style="color:#99f;">' + choose(this.dialogue[what].split('|')) + '</span>"');
		};
